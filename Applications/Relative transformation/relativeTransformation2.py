...
def getPose(device):
	...
	if found:#if we found all the internal corners
		...
		return np.matrix(cameraPosition), np.matrix(rvec)
...
relativeRotation = cameraRotationWebcam - cameraRotationKinect
relativeRotation = cv2.Rodrigues(relativeRotation)[0]
